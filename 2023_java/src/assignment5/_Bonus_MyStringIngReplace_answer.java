...
public class _Bonus_MyStringIngReplace_answer {
	...
	public static String myStringReplace(String strInput, String strTarget, String strReplace) {
	    ...
	    int i = 0;
	    int j = strInput.indexOf(strTarget);
	    
	    String newStr = "";

	    while (j >= 0) {
	    	newStr += strInput.substring(i, j) + strReplace;
	        i = j + strTarget.length();
	        j = strInput.indexOf(strTarget, i);
	    }

	    newStr += strInput.substring(i);
