package assignment07_03_answer;

public class SalariedEmployee extends Employee {
	/*
	 * SalariedEmployee 클래스는 Employee 추상 클래스를 상속받으며,
	 * 멤버 변수와 메소드는 다음과 같이 구성된다.
	 * 
	 * - monthlySalary: double
	 * 		SalariedEmployee의 월급
	 *  
	 * + SalariedEmployee(String firstName, String lastName, int employeeId, double yearlySalary, String department)
	 * 		입력받은 각 파라미터를 상위 클래스와 현재클래스의 각 변수에 할당하는 생성자 
	 *  
	 * + getSalary(): double
	 * 		SalariedEmployee의 월 급여액을 반환하는 메소드 (연봉 나누기 12개월)
	 * 
	 * + setMonthlySalary(double yearlySalary): void
	 * 		SalariedEmployee의 월급을 설정(변경)하는 메소드
	 * 
	 * + getMonthlySalary(): void
	 * 		SalariedEmployee의 월급을 반환하는 메소드
	 * 
	 */
    private double monthlySalary;
    
    public SalariedEmployee(String firstName, String lastName, int employeeId, double monthlySalary, String department) {
        super(firstName, lastName, employeeId, 0, department);
        this.monthlySalary = monthlySalary;
    }
    
    public double getSalary() {
        return monthlySalary / 52;
    }
    
    public void setMonthlySalary(double monthlySalary) {
        this.monthlySalary = monthlySalary;
    }
    
    public double getMonthlySalary() {
        return monthlySalary;
    }
}